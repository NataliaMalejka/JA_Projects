		...
		else // biblioteka ASM
		{
			...
			//for (int numThreads = 1; numThreads <= 64; numThreads *= 2)
			{
				...
				for (int i = 0; i < numThreads; i++)
				{
					...
					int startPixelIndex = currentRow * (width -2) * 3;

					threads.push_back(std::thread(filterAsm, width, height, image_data, new_image_data, currentHeight, currentRow, startPixelIndex));
